...
void setup() {  // This function sets everything up for logging.
  ...
  if (rtc.updateTime() == false) {
    serial.print("RTC failed"); //Updates the time variables from RTC, report if there's a failure
  }
...
void loop() {  //This is the main function. It loops (repeats) forever.
  ...
  //}
    ...
    if (FastRead(DEMOD_OUT_PIN, RFcircuit, checkTime, pollTime1) == 1) {
      ...
      if (rtc.updateTime() == false) serial.print("RTC failed after tag read ");    //Updates the time variables from Real Time Clock
