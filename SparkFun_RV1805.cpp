...
void RV1805::setRptTimer(uint8_t time, uint8_t freq){   //set a timer to repeat a pulse alarm at intervals of time x freq. freq modes 0 = 4096Hz, 1 = 64Hz, 2 = 1Hz, 3 = 1/60 Hz (1 min) 
  ...
  uint8_t value = readRegister(RV1805_CTDWN_TMR_CTRL);           // read current countdown timer control register value
  ...
  writeRegister(RV1805_CTDWN_TMR_CTRL, value);
  //writeRegister(RV1805_INT_MASK, B10001000);              //Timer interrupt enabled (bit 3)
  //The timer is enabled but it must be started with startTimer()
}

void RV1805::startTimer(){
  uint8_t originalTimerControl = readRegister(RV1805_CTDWN_TMR_CTRL);
  writeRegister(RV1805_CTDWN_TMR_CTRL, (B10000000 | originalTimerControl));
}

void RV1805::stopTimer(){
  uint8_t originalTimerControl = readRegister(RV1805_CTDWN_TMR_CTRL);
  writeRegister(RV1805_CTDWN_TMR_CTRL, (B0111111 & originalTimerControl));
}
...
void RV1805::enableLowPower()
{
	...
	writeRegister(RV1805_OSC_CTRL, 0b00000100); //OSEL=1, ACAL=00, BOS=0, FOS=0, IOPW=1, OFIE=0, ACIE=0 // Use crystal oscillator (most accurate)
	//writeRegister(RV1805_OSC_CTRL, 0b11111100); //OSEL=1, ACAL=11, BOS=1, FOS=1, IOPW=1, OFIE=0, ACIE=0	// Use rc ocillator (least accurate)
